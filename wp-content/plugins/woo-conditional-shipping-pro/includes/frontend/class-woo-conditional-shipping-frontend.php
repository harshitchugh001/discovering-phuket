      ...
      if ( $passes ) {
        foreach ( $ruleset->get_actions() as $action_index => $action ) {
          if ( $instance_id !== false && isset( $action['shipping_method_ids'] ) && in_array( $instance_id, (array) $action['shipping_method_ids'] ) ) {
            ...
            $action['price_mode'] = isset( $action['price_mode'] ) ? $action['price_mode'] : '';
            ...
            } else if ( $action['price_mode'] === 'pct_shipping' ) {
              $base = is_array( $args['cost'] ) ? array_sum( $args['cost'] ) : floatval( $args['cost'] );
              $amount = $base * ( floatval( $action['price'] ) / 100 );
            } else {
            ...
            if ( $action['type'] === 'increase_price' ) {
              if ( is_array( $args['cost'] ) ) {
                ...
                $args['cost'] = floatval( $args['cost'] ) + $amount;
              }
            }
